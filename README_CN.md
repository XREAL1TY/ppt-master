...
| [uv](https://docs.astral.sh/uv/) | ✅ **必需** | 包管理器——创建隔离虚拟环境，安装依赖 |

> **一句话总结** — 装好 Python + uv，跑一行 `uv venv skills/ppt-master/.venv && uv pip install -r skills/ppt-master/requirements.txt`，就可以开始生成 PPT 了。
...
> **推荐 — 方式一：npx skills add**（跨 agent CLI，支持 Claude Code、Cursor、Codex 等）
>
> ```bash
...
>
> ```bash
> cd ~/.claude/skills/ppt-master  # (或 ~/.agent/skills/ppt-master) — 即 ${SKILL_DIR}
> uv venv .venv && uv pip install -r requirements.txt
> ```
>
> **执行 `npx skills update` 后：** skill 目录会被整体替换，`.venv` 随之丢失，需重新执行上述两条命令。
>
> **方式二：Claude Code 插件**（仅在 Claude Code 内）
>
> ```bash
...
> ```
>
> 然后从插件安装路径创建 venv（将 `*` 替换为已安装的版本号）：
>
> ```bash
> cd ~/.claude/plugins/cache/ppt-master/ppt-master/*/skills/ppt-master  # 即 ${SKILL_DIR}
> uv venv .venv && uv pip install -r requirements.txt
> ```
>
> **插件更新后：** `.venv` 会丢失，需重新执行上述两条命令。
>
> <details>
> <summary><strong>方式三：Git clone / ZIP</strong> — 完整仓库，适合贡献者或离线使用</summary>
>
> **下载 ZIP**（无需 Git）：[GitHub](https://github.com/XREAL1TY/ppt-master) → **Code → Download ZIP**，解压即用。
>
> **Git clone**（需安装 [Git](https://git-scm.com/downloads)）：
>
> ```bash
